...
elif menu == "Clientes": st.subheader("Cadastro de Cliente") nome = st.text_input("Nome") cpf = st.text_input("CPF") telefone = st.text_input("Telefone") if st.button("Cadastrar Cliente"): cursor.execute("INSERT INTO clientes (nome, cpf, telefone) VALUES (?, ?, ?)", (nome, cpf, telefone)) conn.commit() st.success("Cliente cadastrado com sucesso")

#Cadastro de produtos

elif menu == "Produtos": st.subheader("Cadastro de Produtos") nome = st.text_input("Nome do Produto") preco = st.number_input("Preço", step=0.01) estoque = st.number_input("Estoque", step=1) categoria = st.text_input("Categoria") data = datetime.now().strftime("%Y-%m-%d %H:%M:%S") if st.button("Cadastrar Produto"): cursor.execute("INSERT INTO produtos (nome, preco, estoque, categoria, data) VALUES (?, ?, ?, ?, ?)", (nome, preco, estoque, categoria, data)) conn.commit() st.success("Produto cadastrado com sucesso")
...
elif menu == "Vendas": st.subheader("Registrar Venda") produtos = [row[0] for row in cursor.execute("SELECT nome FROM produtos").fetchall()] clientes = [row[0] for row in cursor.execute("SELECT nome FROM clientes").fetchall()]
