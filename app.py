    ...
    if st.session_state.carrinho:
        ...
        if st.button("Finalizar Venda"):
            ...
            for produto, quantidade in st.session_state.carrinho.items():
                ...
                cursor.execute("""
                    INSERT INTO vendas (data, produto, cliente, quantidade, total, pedido_id, forma_pagamento)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (data_venda, produto, cliente, quantidade, total_produto, pedido_id, forma_pagamento))
            ...
            c.drawString(50, height - 210, f"Forma de Pagamento: {forma_pagamento}")
...
def pagina_relatorios():
    ...
    # Buscar vendas no período filtrado, incluindo forma_pagamento
    cursor.execute("""
        SELECT data, produto, cliente, quantidade, total, forma_pagamento FROM vendas
    ...
    df = pd.DataFrame(vendas, columns=["Data", "Produto", "Cliente", "Quantidade", "Total", "Forma de Pagamento"])
    ...
    def desenhar_cabecalho():
        ...
        pdf.drawString(margem_esquerda + 80, y, "Cliente")
        pdf.drawString(margem_esquerda + 220, y, "Produto")
        pdf.drawRightString(margem_direita - 140, y, "Qtde")
        pdf.drawRightString(margem_direita - 80, y, "Total (R$)")
        pdf.drawRightString(margem_direita - 10, y, "Forma Pgto")
        y -= espacamento_linhas
    ...
    for index, row in df.iterrows():
        ...
        pdf.drawString(margem_esquerda, y, row['Data'][:19])
        pdf.drawString(margem_esquerda + 80, y, str(row['Cliente']))
        pdf.drawString(margem_esquerda + 220, y, str(row['Produto']))
        pdf.drawRightString(margem_direita - 140, y, str(row['Quantidade']))
        pdf.drawRightString(margem_direita - 80, y, f"{row['Total']:.2f}")
        pdf.drawRightString(margem_direita - 10, y, str(row['Forma de Pagamento']))
        y -= espacamento_linhas
